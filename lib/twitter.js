...
const { prepareImageForTwitter } = require("./image");
...
const postSaleTweet = async (sale) => {
  ...
  try {
    if (sale.image && sale.image !== "unknown") {
      ...
      const { buffer, mimeType } = await prepareImageForTwitter(
        Buffer.from(imageResponse.data)
      );
      const mediaId = await rawClient.v1.uploadMedia(buffer, { mimeType });
